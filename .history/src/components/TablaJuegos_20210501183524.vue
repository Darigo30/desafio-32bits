    ...
    <table class="table">
      ...
      <tbody v-if="juegosFiltrados && juegosFiltrados.length === 0">
        <tr class="text-center">
          <td colspan="4" class="py-5 display-6 text-white">No hay stock de juegos</td>
        </tr>
      </tbody>

      <tbody v-else>
        <tr :style="{'background-color': juego.color}" class="text-center" v-for="juego in juegosFiltrados" :key="juego.codigo"
        >
          <td>{{ juego.codigo }}</td>
          <td>{{ juego.nombre }}</td>
          <td>{{ juego.stock }}</td>
